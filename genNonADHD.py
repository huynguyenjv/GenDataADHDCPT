...
NUM_SUBJECTS = 1000        # Số người (file) muốn sinh
NUM_TRIALS = 1200         # Số trial mỗi người
TARGET_RATE = 0.3        # Xác suất xuất hiện Target (VD: 30%)

# Khoảng random CHO NHÓM "KHÔNG ADHD" (thấp)
OMISSION_RANGE = (0.00, 0.10)     # < 10%
COMMISSION_RANGE = (0.00, 0.05)   # < 5%
RT_MEAN_RANGE = (200, 500)        # 200~500ms (phản ứng nhanh)
RT_STD_RANGE  = (20, 80)          # std thấp => ổn định

# Thư mục đầu ra
OUTPUT_DIR = "non_adhd_data_no_label"
...
def generate_cpt_data_for_subject(num_trials, target_rate,
                                  ...
                                  rt_mean, rt_std):
    """
    Tạo danh sách (list) các trial (mỗi trial là 1 dict).
    """
    data_rows = []
